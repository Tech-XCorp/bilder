...
# A summary file will always be generated, but an abstract file will only be generated when
# bilder is invoked with the -H argument.  However, as the files are rotated, their numbers
# need to be kept consistent.
if test -e $SUMMARY -a ! -e $ABSTRACT; then
  # In this case, a summary file exists but an abstract file does not.  Create temporary abstract file so that
  # the rotation will occur, then remove.
  rotateFile $SUMMARY
  touch $ABSTRACT
  rotateFile $ABSTRACT
  rmall ${ABSTRACT}1
else
  # Both a summary and abstract file were previously generated.  Alternatively, neither were
  # generated when the build directory is new.
  rotateFile $SUMMARY
  rotateFile $ABSTRACT
fi
...
# techo "CONFIG_SUPRA_SP_ARG = \"$CONFIG_SUPRA_SP_ARG\""
# techo "CMAKE_SUPRA_SP_ARG = \"$CMAKE_SUPRA_SP_ARG\""
# techo exit; exit
...
if $FORCE_INSTALL; then
  decho "Modified packages will be installed."
fi
