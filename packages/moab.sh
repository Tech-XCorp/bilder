...
# FORPYTHON_STATIC_BUILD needed by composers
  if $MOAB_USE_CMAKE; then
    MOAB_GENCONFIG_ARGS="-DMOAB_USE_CGM:BOOL=FALSE -DMOAB_USE_HDF:BOOL=TRUE -DMOAB_USE_NETCDF:BOOL=TRUE"
# OCE always brought in shared
    MOAB_PYST_CONFIG_ARGS="$MOAB_PYST_CONFIG_ARGS $OCE_PYCSH_CMAKE_DIR_ARG ${MOAB_GENCONFIG_ARGS} -DHDF5_DIR:PATH='${HDF5_PYCST_DIR}' -DNetCDF_DIR:PATH='${NETCDF_PYCST_DIR}'"
    MOAB_PYSH_CONFIG_ARGS="$MOAB_PYSH_CONFIG_ARGS $OCE_PYCSH_CMAKE_DIR_ARG ${MOAB_GENCONFIG_ARGS} -DHDF5_DIR:PATH='${HDF5_PYCSH_DIR}' -DNetCDF_DIR:PATH='${NETCDF_PYCSH_DIR}'"
    MOAB_SER_CONFIG_ARGS="${MOAB_SER_CONFIG_ARGS} ${MOAB_GENCONFIG_ARGS} -DHDF5_DIR:PATH='${HDF5_SER_DIR}' -DNetCDF_DIR:PATH='${NETCDF_SER_DIR}'"
    MOAB_PAR_CONFIG_ARGS="${MOAB_PAR_CONFIG_ARGS} ${MOAB_GENCONFIG_ARGS} -DHDF5_DIR:PATH='${HDF5_PAR_DIR}' -DNetCDF_DIR:PATH='${NETCDF_PAR_DIR}'"
  else
