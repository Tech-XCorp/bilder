...
# zoltan arguments needed for ulixes-par
  ...
  if $MOAB_USE_CMAKE; then
    ...
    MOAB_SERSH_CONFIG_ARGS="-DBUILD_SHARED_LIBS=TRUE $CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER"
...
#
# The configure parameter changes
#
  if $BUILD_EXPERIMENTAL; then
    HDF5_ROOT_VAR=HDF5_ROOT
  else
    HDF5_ROOT_VAR=HDF5_DIR
  fi
...
# FORPYTHON_STATIC_BUILD needed by composers
  if $MOAB_USE_CMAKE; then
    ...
    if echo "$MOAB_BUILD_SET" | grep -q "Full"; then
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE  -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_SER_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_ROOT='$NETCDF_SER_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CMAKE_CGM_PYCSH_DIR/lib/cgm.make' $OCE_SER_CMAKE_DIR_ARG"
      MOAB_SERSH_CONFIG_ARGS="$MOAB_SERSH_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE  -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SERSH_DIR -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_SERSH_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_ROOT='$NETCDF_SERSH_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CMAKE_CGM_PYCSH_DIR/lib/cgm.make' $OCE_SERSH_CMAKE_DIR_ARG"
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PYCST_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CMAKE_CGM_PYCSH_DIR/lib/cgm.make' $OCE_PYCSH_CMAKE_DIR_ARG"
      MOAB_PYCSH_CONFIG_ARGS="$MOAB_PYCSH_CONFIG_ARGS -DBUILD_SHARED_LIBS:BOOL=true -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE -DENABLE_BOOST=FALSE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PYCSH_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CGM_PYCSH_DIR/lib/cgm.make' $OCE_PYCSH_CMAKE_DIR_ARG"
      MOAB_PAR_CONFIG_ARGS="$MOAB_PAR_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PAR_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_ROOT='$NETCDF_SER_DIR' $MOAB_ZOLTAN_ARGS -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CMAKE_CGM_PYCSH_DIR/lib/cgm.make' $OCE_SER_CMAKE_DIR_ARG"
      MOAB_PARSH_CONFIG_ARGS="$MOAB_PARSH_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SERSH_DIR -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PARSH_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_ROOT='$NETCDF_SERSH_DIR' $MOAB_ZOLTAN_ARGS -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CMAKE_CGM_PYCSH_DIR/lib/cgm.make' $OCE_SERSH_CMAKE_DIR_ARG"
    # MOAB_BUILD_SET = Lite only has meshing capabilities
    elif echo "$MOAB_BUILD_SET" | grep -q "Lite"; then
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_SER_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_ROOT='$NETCDF_SER_DIR' -DENABLE_CGM:BOOL=OFF -DMOAB_BUILD_DAGMC:BOOL=OFF"
      MOAB_SERSH_CONFIG_ARGS="$MOAB_SERSH_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SERSH_DIR -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_SERSH_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_ROOT='$NETCDF_SERSH_DIR' -DENABLE_CGM:BOOL=OFF -DMOAB_BUILD_DAGMC:BOOL=OFF"
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DMOAB_USE_HDF:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PYCST_DIR'"
      MOAB_PYCSH_CONFIG_ARGS="$MOAB_PYCSH_CONFIG_ARGS -DMOAB_USE_HDF:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PYCSH_DIR'"
      ...
      MOAB_PARSH_CONFIG_ARGS="$MOAB_PARSH_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SERSH_DIR -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PARSH_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_ROOT='$NETCDF_SERSH_DIR' -DPNETCDF_ROOT='$NETCDF_PARSH_DIR' -DENABLE_DAGMC:BOOL=OFF -DENABLE_CGM:BOOL=OFF -DMOAB_BUILD_MBPART:BOOL=ON -DMOAB_BUILD_DAGMC:BOOL=OFF $MOAB_ZOLTAN_ARGS"
    else
# Default choice
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_SER_DIR' -DENABLE_CGM:BOOL=OFF"
# HDF5_DIR -> HDF5_ROOT with latest moab.  I think others need updating, but
# making minimal change for now.
      MOAB_SERSH_CONFIG_ARGS="$MOAB_SERSH_CONFIG_ARGS -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_SERSH_DIR' -DENABLE_CGM:BOOL=OFF"
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PYCST_DIR' -DENABLE_CGM:BOOL=OFF"
      MOAB_PYCSH_CONFIG_ARGS="$MOAB_PYCSH_CONFIG_ARGS -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PYCSH_DIR' -DENABLE_CGM:BOOL=OFF"
      MOAB_PAR_CONFIG_ARGS="$MOAB_PAR_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PAR_DIR' -DENABLE_CGM:BOOL=OFF -DMOAB_BUILD_MBPART:BOOL=ON $MOAB_ZOLTAN_ARGS"
      MOAB_PARSH_CONFIG_ARGS="$MOAB_PARSH_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_HDF5:BOOL=TRUE '-D${HDF5_ROOT_VAR}:PATH=$HDF5_PARSH_DIR' -DENABLE_CGM:BOOL=OFF -DMOAB_BUILD_MBPART:BOOL=ON $MOAB_ZOLTAN_ARGS"
    fi
  else
