...
# MOAB uses root dirs for dirs.
    if echo "$MOAB_BUILD_SET" | grep -q "full"; then
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_SER_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_SER_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_DIR='$CGM_SER_DIR' $OCE_SER_CMAKE_DIR_ARG"
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PYCST_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_DIR='$CGM_PYSH_DIR' $OCE_PYCSH_CMAKE_DIR_ARG"
      MOAB_4PYSH_CONFIG_ARGS="$MOAB_PYSH_CONFIG_ARGS -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PYCSH_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_DIR='$CGM_PYSH_DIR' $OCE_PYCSH_CMAKE_DIR_ARG"
      MOAB_PAR_CONFIG_ARGS="$MOAB_PAR_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PAR_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_SER_DIR' $MOAB_ZOLTAN_ARGS -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_DIR='$CGM_SER_DIR' $OCE_SER_CMAKE_DIR_ARG"
    else
      ...
      MOAB_4PYSH_CONFIG_ARGS="$MOAB_PYSH_CONFIG_ARGS -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DMOAB_USE_HDF:BOOL=TRUE -DHdf5_ROOT_DIR:PATH='$HDF5_PYCSH_DIR'"
...
# Python shared build for composers.  Build with cmake.
  ...
  if bilderConfig $makerargs -c moab $FORPYTHON_SHARED_BUILD${moabBuildSet} "-DBUILD_SHARED_LIBS:BOOL=true $CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER -DMOAB_USE_HDF:BOOL=TRUE -DHdf5_ROOT_DIR:PATH='$HDF5_PYCSH_DIR' $OCE_PYCSH_CMAKE_DIR_ARG $otherargs" "" "$MOAB_ENV"; then
