...
# MOAB uses root dirs for dirs.
    if echo "$MOAB_BUILD_SET" | grep -q "full"; then
      ...
      MOAB_PYCSH_CONFIG_ARGS="$MOAB_PYCSH_CONFIG_ARGS -DENABLE_BOOST=FALSE -DBOOST_ROOT=$BOOST_PYCSH_DIR -DMOAB_USE_HDF:BOOL=TRUE -DHdf5_ROOT_DIR:PATH='$HDF5_PYCSH_DIR'"
...
# Python shared build for composers.  Build with cmake.
  ...
  if bilderConfig $makerargs -c moab $FORPYTHON_SHARED_BUILD${moabBuildSet} "$MOAB_PYCSH_CONFIG_ARGS $MOAB_PYCSH_OTHER_ARGS" "" "$MOAB_ENV"; then
