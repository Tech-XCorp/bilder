...
# par BUILD needed by ulixes
  ...
  if $MOAB_USE_CMAKE; then
    ...
    MOAB_PAR_CONFIG_ARGS="-DBUILD_SHARED_LIBS:BOOL=FALSE $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR -DENABLE_PARALLEL:BOOL=TRUE"
  else
  ...
  if echo "$MOAB_BUILDS" | grep -q "par"; then
      ...
      if [[ ${CMAKE_TRILINOS_PARCOMMIO_DIR} != "" ]]; then
	  MOAB_ZOLTAN_ARGS="-DENABLE_ZOLTAN:BOOL=TRUE -DZOLTAN_DIR=${CMAKE_TRILINOS_PARCOMMIO_DIR}"
      elif [[ ${CMAKE_TRILINOS_PARCOMM_DIR} != "" ]]; then
	  MOAB_ZOLTAN_ARGS="-DENABLE_ZOLTAN:BOOL=TRUE -DZOLTAN_DIR=${CMAKE_TRILINOS_PARCOMM_DIR}"
      elif [[ ${CMAKE_TRILINOS_PARFULL_DIR} != "" ]]; then
	  MOAB_ZOLTAN_ARGS="-DENABLE_ZOLTAN:BOOL=TRUE -DZOLTAN_DIR=${CMAKE_TRILINOS_PARFULL_DIR}"
      else
...
# FORPYTHON_STATIC_BUILD needed by composers
  if $MOAB_USE_CMAKE; then
    MOAB_GENCONFIG_ARGS="-DENABLE_IMESH:BOOL=TRUE"
# OCE always brought in shared
# MOAB uses root dirs for dirs.
    if echo "$MOAB_BUILD_SET" | grep -q "full"; then
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_SER_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_SER_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_DIR='$CGM_SER_DIR' $OCE_SER_CMAKE_DIR_ARG"
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PYCST_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_DIR='$CGM_PYSH_DIR' $OCE_PYCSH_CMAKE_DIR_ARG"
      MOAB_4PYSH_CONFIG_ARGS="$MOAB_PYSH_CONFIG_ARGS -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PYCSH_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_DIR='$CGM_PYSH_DIR' $OCE_PYCSH_CMAKE_DIR_ARG"
      MOAB_PAR_CONFIG_ARGS="$MOAB_PAR_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PAR_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_PAR_DIR' $MOAB_ZOLTAN_ARGS -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_DIR='$CGM_SER_DIR' $OCE_SER_CMAKE_DIR_ARG"
    else
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_SER_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_SER_DIR' -DENABLE_DAGMC:BOOL=OFF -DENABLE_CGM:BOOL=OFF"
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DMOAB_USE_HDF:BOOL=TRUE -DHdf5_ROOT_DIR:PATH='$HDF5_PYCST_DIR'"
      MOAB_4PYSH_CONFIG_ARGS="$MOAB_PYSH_CONFIG_ARGS -DMOAB_USE_HDF:BOOL=TRUE -DHdf5_ROOT_DIR:PATH='$HDF5_PYCSH_DIR'"
      MOAB_PAR_CONFIG_ARGS="$MOAB_PAR_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PAR_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_PAR_DIR' -DPNETCDF_DIR='$NETCDF_PAR_DIR' -DENABLE_DAGMC:BOOL=OFF -DENABLE_CGM:BOOL=OFF $MOAB_ZOLTAN_ARGS"
    fi
  else
