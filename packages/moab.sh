...
# MOAB uses root dirs for dirs.
    if echo "$MOAB_BUILD_SET" | grep -q "full"; then
      ...
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_PYCST_DIR -DMOAB_USE_HDF:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PYCST_DIR'"
      MOAB_PYCSH_CONFIG_ARGS="$MOAB_PYCSH_CONFIG_ARGS -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_PYCSH_DIR -DMOAB_USE_HDF:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PYCSH_DIR'"
