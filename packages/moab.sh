...
# MOAB uses root dirs for dirs.
    if echo "$MOAB_BUILD_SET" | grep -q "full"; then
      ...
      MOAB_PYCSH_CONFIG_ARGS="$MOAB_PYCSH_CONFIG_ARGS -DBUILD_SHARED_LIBS:BOOL=true -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE -DENABLE_BOOST=FALSE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_ROOT:PATH='$HDF5_PYCSH_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=OFF $OCE_PYCSH_CMAKE_DIR_ARG"
      ...
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_ROOT:PATH='$HDF5_SER_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_ROOT='$NETCDF_SER_DIR' -DENABLE_CGM:BOOL=OFF -DMOAB_BUILD_DAGMC:BOOL=OFF"
