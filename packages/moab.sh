...
# When not all dependencies right on Windows, need nmake
  ...
  local moabBuildSet=""
  if [[ $MOAB_BUILD_SET = lite || $MOAB_BUILD_SET = liteForPython ]]; then
    moabBuildSet=Lite
  fi
...
# Python shared build for composers.  Build with cmake.
  ...
  if bilderConfig $makerargs $moabcmakearg moab pycsh${moabBuildSet} "$MOAB_PYCSH_CONFIG_ARGS $MOAB_PYCSH_OTHER_ARGS" "" "$MOAB_ENV"; then
