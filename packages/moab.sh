...
# Get moab from repo, determine whether to build
  ...
  local CMAKE_COMP_WIN_FLAGS=""
  local MOAB_SER_WIN_ARGS=""
  local MOAB_PAR_WIN_ARGS=""
  if [[ `uname` =~ CYGWIN ]]; then
      if echo "$CMAKE_COMPILERS_SER" | grep -q "icl"; then
	  CMAKE_COMP_WIN_FLAGS="-DCMAKE_CXX_FLAGS_DEBUG:STRING='/D_DEBUG /Zi /Ob0 /Od /RTC1' -DCMAKE_CXX_FLAGS_MINSIZEREL:STRING='/O1 /Ob1 /D NDEBUG' -DCMAKE_CXX_FLAGS_RELEASE:STRING='/O2 /Ob2 /D NDEBUG' -DCMAKE_CXX_FLAGS_RELWITHDEBINFO:STRING='/Zi /O2 /Ob1 /D NDEBUG' -DCMAKE_C_FLAGS_DEBUG:STRING='/D_DEBUG /Zi /Ob0 /Od /RTC1' -DCMAKE_C_FLAGS_MINSIZEREL:STRING='/O1 /Ob1 /D NDEBUG' -DCMAKE_C_FLAGS_RELEASE:STRING='/O2 /Ob2 /D NDEBUG' -DCMAKE_C_FLAGS_RELWITHDEBINFO:STRING='/Zi /O2 /Ob1 /D NDEBUG'" 
	  MOAB_SER_WIN_ARGS="-DCMAKE_STATIC_LINKER_FLAGS:STRING='/NODEFAULTLIB:msvcrt' -DCMAKE_EXE_LINKER_FLAGS:STRING='/NODEFAULTLIB:msvcrt' -DMOAB_FORCE_32_BIT_HANDLES:BOOL=TRUE ${CMAKE_COMP_WIN_FLAGS}"
      fi
      if echo "$CMAKE_COMPILERS_PAR" | grep -q "mpiicc"; then
	  MOAB_PAR_WIN_ARGS="-DCMAKE_STATIC_LINKER_FLAGS:STRING='/NODEFAULTLIB:msvcrt' -DCMAKE_EXE_LINKER_FLAGS:STRING='/NODEFAULTLIB:msvcrt' -DMPI_HEADER_PATH:PATH='C:/Program Files (x86)/Intel/MPI/5.0.3.048/intel64/include'  -DMPI_DIR:PATH='C:/Program Files (x86)/Intel/MPI/5.0.3.048/intel64/'  -DMPI_INCLUDE_PATH:PATH='C:/Program Files (x86)/Intel/MPI/5.0.3.048/intel64/include' -DMPI_LIBRARY:FILEPATH:FILEPATH='C:/Program Files (x86)/Intel/MPI/5.0.3.048/intel64/lib/release_mt/impi.lib' -DMOAB_FORCE_32_BIT_HANDLES:BOOL=TRUE ${CMAKE_COMP_WIN_FLAGS}"
      fi
  fi
...
# par BUILD needed by ulixes
  ...
  if $MOAB_USE_CMAKE; then
    MOAB_SER_CONFIG_ARGS="-DBUILD_SHARED_LIBS=FALSE $CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $MOAB_SER_WIN_ARGS"
#    if [[ `uname` =~ CYGWIN ]]; then
...
#    fi
    ...
    MOAB_4PYSH_CONFIG_ARGS="-DBUILD_SHARED_LIBS:BOOL=TRUE -DCMAKE_INSTALL_NAME_DIR='${BLDR_INSTALL_DIR}/moab-${MOAB_BLDRVERSION}-$FORPYTHON_SHARED_BUILD/lib' $CMAKE_COMPILERS_PYC $CMAKE_COMPFLAGS_PYFLAGS"
    MOAB_PAR_CONFIG_ARGS="-DBUILD_SHARED_LIBS=FALSE $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $MOAB_PAR_WIN_ARGS -DENABLE_PARALLEL:BOOL=TRUE"
  else
  ...
  if echo "$MOAB_BUILDS" | grep -q "par"; then
      # Pick parcommio as the default as that includes hdf5/netcdf/exodusII API's.
...
# MOAB uses root dirs for dirs.
    if echo "$MOAB_BUILD_SET" | grep -q "full"; then
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE  -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_SER_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_SER_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CMAKE_CGM_SERSH_DIR/lib/cgm.make' $OCE_SER_CMAKE_DIR_ARG"
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PYCST_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CMAKE_CGM_SERSH_DIR/lib/cgm.make' $OCE_PYCSH_CMAKE_DIR_ARG"
      MOAB_4PYSH_CONFIG_ARGS="$MOAB_PYSH_CONFIG_ARGS -DBUILD_SHARED_LIBS:BOOL=true -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE -DENABLE_BOOST=FALSE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PYCSH_DIR' -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CGM_PYCSH_DIR/lib/cgm.make' $OCE_PYCSH_CMAKE_DIR_ARG"
      MOAB_PAR_CONFIG_ARGS="$MOAB_PAR_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_IMESH:BOOL=TRUE -DENABLE_IGEOM:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PAR_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_SER_DIR' $MOAB_ZOLTAN_ARGS -DENABLE_DAGMC:BOOL=ON -DENABLE_CGM:BOOL=ON -DCGM_CFG='$CMAKE_CGM_SERSH_DIR/lib/cgm.make' $OCE_SER_CMAKE_DIR_ARG"
    else
      MOAB_SER_CONFIG_ARGS="$MOAB_SER_CONFIG_ARGS -DENABLE_IMESH:BOOL=TRUE -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_SER_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_SER_DIR' -DENABLE_DAGMC:BOOL=OFF -DENABLE_CGM:BOOL=OFF"
      MOAB_PYCST_CONFIG_ARGS="$MOAB_PYCST_CONFIG_ARGS -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DMOAB_USE_HDF:BOOL=TRUE -DHdf5_ROOT_DIR:PATH='$HDF5_PYCST_DIR'"
      MOAB_4PYSH_CONFIG_ARGS="$MOAB_PYSH_CONFIG_ARGS -DENABLE_BOOST=FALSE -DBOOST_ROOT=$BOOST_SERSH_DIR -DMOAB_USE_HDF:BOOL=TRUE -DHdf5_ROOT_DIR:PATH='$HDF5_PYCSH_DIR'"
      MOAB_PAR_CONFIG_ARGS="$MOAB_PAR_CONFIG_ARGS -DMPI_DIR:PATH='$CONTRIB_DIR/mpi' -DENABLE_BOOST=TRUE -DBOOST_ROOT=$BOOST_SER_DIR -DENABLE_IMESH:BOOL=TRUE -DENABLE_HDF5:BOOL=TRUE -DHDF5_DIR:PATH='$HDF5_PAR_DIR' -DENABLE_NETCDF:BOOL=TRUE -DNETCDF_DIR='$NETCDF_SER_DIR' -DPNETCDF_DIR='$NETCDF_PAR_DIR' -DENABLE_DAGMC:BOOL=OFF -DENABLE_CGM:BOOL=OFF $MOAB_ZOLTAN_ARGS"
    fi
  else
...
# Configure and build serial
  if bilderConfig $makerargs $moabcmakearg moab ser${moabBuildSet} "$MOAB_SER_CONFIG_ARGS $MOAB_SER_OTHER_ARGS" "" "$MOAB_ENV"; then
...
# Cannot use FORPYTHON_STATIC_BUILD on unixish where it can resolve to ser,
  if bilderConfig $makerargs $moabcmakearg moab pycst${moabBuildSet} "$MOAB_PYCST_CONFIG_ARGS $MOAB_PYCST_OTHER_ARGS" "" "$MOAB_ENV"; then
...
# Python shared build for composers.  Build with cmake.
  ...
  if bilderConfig $makerargs -c moab $FORPYTHON_SHARED_BUILD${moabBuildSet} "$MOAB_4PYSH_CONFIG_ARGS $MOAB_4PYSH_OTHER_ARGS" "" "$MOAB_ENV"; then
    bilderBuild $makerargs moab $FORPYTHON_SHARED_BUILD${moabBuildSet} "$makejargs" "$MOAB_ENV"
  fi

# Configure and build parallel
  if bilderConfig $makerargs $moabcmakearg moab par${moabBuildSet} "$MOAB_PAR_CONFIG_ARGS $MOAB_PAR_OTHER_ARGS" "" "$MOAB_ENV"; then
