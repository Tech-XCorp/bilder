...
# @file    python.sh
#
# @brief   Build information for python.
#
# @version $Rev$ $Date$
#
...
# 2.7.13
# With more recent version of the cmake, the name of the flag for the
# shared build changed.  Setting both.
      PYTHON_PYCSH_ADDL_ARGS="$PYTHON_PYCSH_ADDL_ARGS -DBUILD_SHARED=ON -DBUILD_LIBPYTHON_SHARED:BOOL=ON -DBUILD_STATIC=OFF -DINSTALL_WINDOWS_TRADITIONAL=ON -DPYTHON_VERSION=$PY_DL_VERSION"
...
# On Windows, build is static and needs to go into contrib
  ...
  if bilderConfig $pyconfflags -I $CONTRIB_DIR Python $FORPYTHON_SHARED_BUILD "$PYTHON_PYCSH_ADDL_ARGS $PYTHON_PYCSH_OTHER_ARGS"; then
