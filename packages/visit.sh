...
# hdf5 remove dll from library names as of 1.8.11
  ...
  local VISIT_OS_ARGS=
  VISIT_COMPILERS_SER="-DCMAKE_C_COMPILER:FILEPATH='$CC' -DCMAKE_CXX_COMPILER:FILEPATH='$CXX'"
  VISIT_COMPILERS_PYC="-DCMAKE_C_COMPILER:FILEPATH='$PYC_CC' -DCMAKE_CXX_COMPILER:FILEPATH='$PYC_CXX'"
  VISIT_COMPFLAGS_SER="CFLAGS='$CFLAGS' CXXFLAGS='$CXXFLAGS'"
  VISIT_COMPFLAGS_PYC="CFLAGS='$PYCCFLAGS' CXXFLAGS='$PYC_CXXFLAGS'"
  case `uname` in
...
# But it appears that on snowleopard with need to add the library dir?
    Darwin)
      ...
      VISIT_COMPFLAGS_SER="CFLAGS='$CFLAGS' CXXFLAGS='$CXXFLAGS -Wno-error=c++11-narrowing'"
      VISIT_COMPFLAGS_PYC="CFLAGS='$PYCCFLAGS' CXXFLAGS='$PYC_CXXFLAGS -Wno-error=c++11-narrowing'"
...
# Only one of these serial builds will be chosen
  ...
  if bilderConfig -S 60 -c visit sersh "-DBUILDNAME:STRING='${RUNNRSYSTEM}-${BILDER_CHAIN}-sersh' -DIGNORE_THIRD_PARTY_LIB_PROBLEMS:BOOL=ON -DVISIT_INSTALL_THIRD_PARTY:BOOL=ON -DBUILD_SHARED_LIBS:BOOL=ON $VISIT_COMPILERS_SER $VISIT_COMPFLAGS_SER $VISIT_PKG_ARGS $VISIT_OS_ARGS $VISIT_ADDL_ARGS $VISIT_SERSH_OTHER_ARGS" "" "$VISIT_ENV"; then
    bilderBuild visit sersh "$VISIT_MAKE_ARGS" "$VISIT_ENV"
  fi

  if bilderConfig -S 60 -c visit pycsh "-DBUILDNAME:STRING='${RUNNRSYSTEM}-${BILDER_CHAIN}-pycsh' -DIGNORE_THIRD_PARTY_LIB_PROBLEMS:BOOL=ON -DVISIT_INSTALL_THIRD_PARTY:BOOL=ON -DBUILD_SHARED_LIBS:BOOL=ON $VISIT_COMPILERS_PYC $VISIT_COMPFLAGS_PYC $VISIT_PKG_ARGS $VISIT_OS_ARGS $VISIT_ADDL_ARGS $VISIT_PYCSH_OTHER_ARGS" "" "$VISIT_ENV"; then
    bilderBuild visit pycsh "$VISIT_MAKE_ARGS" "$VISIT_ENV"
  fi

# Build parallel doing optional builds
  if bilderConfig -S 60 -c visit parsh "-DBUILDNAME:STRING='${RUNNRSYSTEM}-${BILDER_CHAIN}-parsh' -DVISIT_PARALLEL:BOOL=ON -DVISIT_MPI_COMPILER='$MPICXX' -DVISIT_MPI_LIBS:PATH=$MPI_LIBDIR -DIGNORE_THIRD_PARTY_LIB_PROBLEMS:BOOL=ON -DVISIT_INSTALL_THIRD_PARTY:BOOL=ON -DBUILD_SHARED_LIBS:BOOL=ON $VISIT_COMPILERS_PAR $VISIT_COMPFLAGS_PAR $VISIT_PKG_ARGS $VISIT_OS_ARGS $VISIT_ADDL_ARGS $VISIT_PARSH_OTHER_ARGS" "" "$VISIT_ENV"; then
