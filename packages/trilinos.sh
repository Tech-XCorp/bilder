...
# The packages
  ...
  triTplSerArgs=`getTriTPLs ser "Hypre MUMPS SuperLU SuperLU_DIST"`
  triTplParArgs=`getTriTPLs par "Hypre MUMPS SuperLU SuperLU_DIST"`
...
# The builds
  if bilderConfig trilinos sercomm "$CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $triCommonArgs $TRILINOS_ALL_ADDL_ARGS $TRILINOS_SERCOMM_ADDL_ARGS $TRILINOS_SERCOMM_OTHER_ARGS"; then
    bilderBuild trilinos sercomm "$TRILINOS_MAKEJ_ARGS"
  fi
  if bilderConfig trilinos sercommsh "-DBUILD_SHARED_LIBS:BOOL=ON $CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $triCommonArgs $TRILINOS_ALL_ADDL_ARGS $TRILINOS_SERCOMMSH_ADDL_ARGS $TRILINOS_SERCOMMSH_OTHER_ARGS"; then
    bilderBuild trilinos sercommsh "$TRILINOS_MAKEJ_ARGS"
  fi
  if bilderConfig trilinos parcomm "-DTPL_ENABLE_MPI:BOOL=ON $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $triCommonArgs $TRILINOS_ALL_ADDL_ARGS $TRILINOS_PARCOMM_ADDL_ARGS $TRILINOS_PARCOMM_OTHER_ARGS"; then
    bilderBuild trilinos parcomm "$TRILINOS_MAKEJ_ARGS"
  fi
  if bilderConfig trilinos parcommsh "-DTPL_ENABLE_MPI:BOOL=ON -DBUILD_SHARED_LIBS:BOOL=ON $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $triCommonArgs $TRILINOS_ALL_ADDL_ARGS $TRILINOS_PARCOMMSH_ADDL_ARGS $TRILINOS_PARCOMMSH_OTHER_ARGS"; then
    bilderBuild trilinos parcommsh "$TRILINOS_MAKEJ_ARGS"
  fi
# ben is comm for back end nodes
  if bilderConfig trilinos ben "-DTPL_ENABLE_MPI:BOOL=ON $CMAKE_COMPILERS_BEN $CMAKE_COMPFLAGS_BEN $triCommonArgs $TRILINOS_ALL_ADDL_ARGS $TRILINOS_BEN_ADDL_ARGS $TRILINOS_BEN_OTHER_ARGS"; then
...
# The builds
  if bilderConfig trilinos sercommio "$CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $triCommonArgs $TRILINOS_ALL_ADDL_ARGS $TRILINOS_SERCOMMIO_ADDL_ARGS $TRILINOS_SERCOMMIO_OTHER_ARGS"; then
    bilderBuild trilinos sercommio "$TRILINOS_MAKEJ_ARGS"
  fi
  if bilderConfig trilinos parcommio "-DTPL_ENABLE_MPI:BOOL=ON $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $triCommonArgs $TRILINOS_ALL_ADDL_ARGS $TRILINOS_PARCOMMIO_ADDL_ARGS $TRILINOS_PARCOMMIO_OTHER_ARGS"; then
...
#
# The args
  local TRILINOS_FULL_ARGS="$triCommonArgs -DTPL_ENABLE_PARMETIS:BOOL=ON"
