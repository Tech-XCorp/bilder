...
#
# The packages
  local TRILINOS_COMM_ARGS="-DTrilinos_ENABLE_ML:BOOL=ON -DTrilinos_ENABLE_AztecOO:BOOL=ON -DTPL_ENABLE_SuperLU:BOOL=ON -DTrilinos_ENABLE_Amesos:BOOL=ON -DTrilinos_ENABLE_Galeri:BOOL=ON -DTrilinos_ENABLE_Shards:BOOL=ON -DTrilinos_ENABLE_Intrepid:BOOL=ON -DTrilinos_ENABLE_Komplex:BOOL=ON -DTrilinos_ENABLE_Phalanx:BOOL=ON -DTrilinos_ENABLE_NOX:STRING=ON -DTrilinos_ENABLE_EpetraExt:BOOL=ON -DTrilinos_ENABLE_Epetra:BOOL=ON -DTrilinos_ENABLE_Triutils:BOOL=ON -DTrilinos_ENABLE_Teuchos:BOOL=ON -DTeuchos_ENABLE_LONG_LONG_INT:BOOL=ON -DTrilinos_ENABLE_Ifpack:BOOL=ON -DTPL_ENABLE_BinUtils:BOOL=OFF -DTPL_ENABLE_Boost:STRING=ON"
# Add in the locations by build
# SuperLU serial and parallal libraries have different names
  TRILINOS_SERCOMM_ADDL_ARGS="$TRILINOS_SER_ADDL_ARGS -DTPL_ENABLE_SuperLU:BOOL=ON -DSuperLU_INCLUDE_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}-ser/include' -DSuperLU_LIBRARY_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}-ser/lib' -DSuperLU_LIBRARY_NAMES:STRING='superlu'"
  TRILINOS_SERCOMMSH_ADDL_ARGS="$TRILINOS_SERSH_ADDL_ARGS -DTPL_ENABLE_SuperLU:BOOL=ON -DSuperLU_INCLUDE_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}-sersh/include' -DSuperLU_LIBRARY_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}-sersh/lib' -DSuperLU_LIBRARY_NAMES:STRING='superlu'"
  TRILINOS_PARCOMM_ADDL_ARGS="$TRILINOS_PAR_ADDL_ARGS -DTPL_ENABLE_SuperLUDistDist:BOOL=ON -DSuperLUDist_INCLUDE_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}-par/include' -DSuperLUDist_LIBRARY_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}-par/lib' -DSuperLUDist_LIBRARY_NAMES:STRING='superlu_dist'"
  TRILINOS_PARCOMMSH_ADDL_ARGS="$TRILINOS_PARSH_ADDL_ARGS -DTPL_ENABLE_SuperLUDistDist:BOOL=ON -DSuperLUDist_INCLUDE_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}parsh/include' -DSuperLUDist_LIBRARY_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}parsh/lib' -DSuperLUDist_LIBRARY_NAMES:STRING='superlu_dist'"
  TRILINOS_BEN_ADDL_ARGS="$TRILINOS_PAR_ADDL_ARGS -DSuperLU_INCLUDE_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}-ben/include' -DSuperLU_LIBRARY_DIRS:PATH='$CONTRIB_DIR/superlu-${SUPERLU_BLDRVERSION}-ben/lib' -DSuperLU_LIBRARY_NAMES:STRING='superlu_dist'"
# The builds
  if bilderConfig trilinos sercomm "$CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $TRILINOS_COMM_ARGS $TRILINOS_ALL_ADDL_ARGS $TRILINOS_SERCOMM_ADDL_ARGS $TRILINOS_SERCOMM_OTHER_ARGS"; then
    bilderBuild trilinos sercomm "$TRILINOS_MAKEJ_ARGS"
  fi
  if bilderConfig trilinos sercommsh "-DBUILD_SHARED_LIBS:BOOL=ON $CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $TRILINOS_COMM_ARGS $TRILINOS_ALL_ADDL_ARGS $TRILINOS_SERCOMMSH_ADDL_ARGS $TRILINOS_SERCOMMSH_OTHER_ARGS"; then
    bilderBuild trilinos sercommsh "$TRILINOS_MAKEJ_ARGS"
  fi
  if bilderConfig trilinos parcomm "-DTPL_ENABLE_MPI:BOOL=ON $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $TRILINOS_COMM_ARGS $TRILINOS_ALL_ADDL_ARGS $TRILINOS_PARCOMM_ADDL_ARGS $TRILINOS_PARCOMM_OTHER_ARGS"; then
    bilderBuild trilinos parcomm "$TRILINOS_MAKEJ_ARGS"
  fi
  if bilderConfig trilinos parcommsh "-DTPL_ENABLE_MPI:BOOL=ON -DBUILD_SHARED_LIBS:BOOL=ON $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $TRILINOS_COMM_ARGS $TRILINOS_ALL_ADDL_ARGS $TRILINOS_PARCOMMSH_ADDL_ARGS $TRILINOS_PARCOMMSH_OTHER_ARGS"; then
    bilderBuild trilinos parcommsh "$TRILINOS_MAKEJ_ARGS"
  fi
# ben is comm for back end nodes
  if bilderConfig trilinos ben "-DTPL_ENABLE_MPI:BOOL=ON $CMAKE_COMPILERS_BEN $CMAKE_COMPFLAGS_BEN $TRILINOS_COMM_ARGS $TRILINOS_ALL_ADDL_ARGS $TRILINOS_BEN_ADDL_ARGS $TRILINOS_BEN_OTHER_ARGS"; then
...
# The args
  ...
  TRILINOS_PARCOMMIO_ADDL_ARGS="$TRILINOS_PARCOMM_ADDL_ARGS $SEACAS_ARGS"
# The builds
  if bilderConfig trilinos sercommio "$CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $TRILINOS_COMM_ARGS $TRILINOS_ALL_ADDL_ARGS $TRILINOS_SERCOMMIO_ADDL_ARGS $TRILINOS_SERCOMMIO_OTHER_ARGS"; then
    bilderBuild trilinos sercommio "$TRILINOS_MAKEJ_ARGS"
  fi
  if bilderConfig trilinos parcommio "-DTPL_ENABLE_MPI:BOOL=ON $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $TRILINOS_COMM_ARGS $TRILINOS_ALL_ADDL_ARGS $TRILINOS_PARCOMMIO_ADDL_ARGS $TRILINOS_PARCOMMIO_OTHER_ARGS"; then
    bilderBuild trilinos parcommio "$TRILINOS_MAKEJ_ARGS"
  fi

#
# The full builds include non commercially licensable libraries.
# AFAICT, only parmetis
#
# The args
  local TRILINOS_FULL_ARGS="$TRILINOS_COMM_ARGS -DTPL_ENABLE_PARMETIS:BOOL=ON"
  TRILINOS_PARFULL_ADDL_ARGS="$TRILINOS_PARCOMM_ADDL_ARGS -DParMETIS_INCLUDE_DIRS:PATH='$CONTRIB/parmetis-${PARMETIS_BLDRVERSION}-par/include' -DParMETIS_LIBRARY_DIRS:PATH='$CONTRIB/parmetis-${PARMETIS_BLDRVERSION}-par/lib'"
  TRILINOS_PARFULLSH_ADDL_ARGS="$TRILINOS_PARCOMMSH_ADDL_ARGS -DParMETIS_INCLUDE_DIRS:PATH='$CONTRIB/parmetis-${PARMETIS_BLDRVERSION}-par/include' -DParMETIS_LIBRARY_DIRS:PATH='$CONTRIB/parmetis-${PARMETIS_BLDRVERSION}-par/lib'"
# The builds
