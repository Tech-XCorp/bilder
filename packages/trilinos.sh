...
# Back-end node linear algebra libraries
  if test -n "$BLAS_BEN_STATIC_LIBRARIES" -a -n "$LAPACK_BEN_STATIC_LIBRARIES"; then
    TRILINOS_BEN_LINLIB_ARGS="-DTPL_BLAS_LIBRARIES:FILEPATH='`echo $BLAS_BEN_STATIC_LIBRARIES | tr ' ' ';'`'"
    TRILINOS_BEN_LINLIB_ARGS="$TRILINOS_BEN_LINLIB_ARGS -DTPL_LAPACK_LIBRARIES:FILEPATH='`echo $LAPACK_BEN_STATIC_LIBRARIES | tr ' ' ';'`'"
  else
