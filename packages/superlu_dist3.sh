...
buildSuperlu_Dist3() {
  ...
  if bilderConfig -c superlu_dist3 par "-DENABLE_PARALLEL:BOOL=TRUE $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $CMAKE_HDF5_PAR_DIR_ARG $CMAKE_SUPRA_SP_ARG $SUPERLU_DIST3_PAR_OTHER_ARGS"; then
    bilderBuild superlu_dist3 par
  fi
  if bilderConfig superlu_dist3 parsh "-DENABLE_PARALLEL:BOOL=TRUE -DBUILD_SHARED_LIBS:BOOL=ON $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $CMAKE_HDF5_PAR_DIR_ARG $CMAKE_SUPRA_SP_ARG $SUPERLU_DIST3_PARSH_OTHER_ARGS"; then
  ...
  if bilderConfig -c superlu_dist3 ben "-DENABLE_PARALLEL:BOOL=TRUE -DDISABLE_CPUCHECK:BOOL=TRUE $CMAKE_COMPILERS_BEN $CMAKE_COMPFLAGS_BEN $CMAKE_HDF5_BEN_DIR_ARG $CMAKE_SUPRA_SP_ARG $SUPERLU_DIST3_BEN_OTHER_ARGS"; then
