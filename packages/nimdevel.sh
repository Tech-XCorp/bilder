...
if $NIMDEVEL_WITH_OPENMP; then
  NIMDEVEL_DEPS=${NIMDEVEL_DEPS}",superlu_dist3,superlu_dist5,gpreftools"
...
if $NIMDEVEL_ALLSOLVERS; then
  NIMDEVEL_DEPS=${NIMDEVEL_DEPS}",mumps,hypre"
  NIMDEVEL_PAR_OTHER_ARGS="$NIMDEVEL_PAR_OTHER_ARGS -DENABLE_Mumps:BOOL=TRUE -DENABLE_Hypre:BOOL=TRUE"
  NIMDEVEL_SER_OTHER_ARGS="$NIMDEVEL_SER_OTHER_ARGS -DENABLE_Mumps:BOOL=TRUE"
else
  NIMDEVEL_PAR_OTHER_ARGS="$NIMDEVEL_PAR_OTHER_ARGS -DENABLE_Mumps:BOOL=FALSE -DENABLE_Hypre:BOOL=FALSE"
