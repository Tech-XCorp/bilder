...
# Make targets modified according to testing
  ...
  if test -n "$TXBASE_CTEST_TARGET"; then
    ...
    TXBASE_MAKE_ARGS="${TXBASE_CTEST_TARGET}Start ${TXBASE_CTEST_TARGET}Build"
...
# All builds
  ...
  if bilderConfig -c txbase cc4py "-DBUILD_WITH_SHARED_RUNTIME:BOOL=TRUE $CMAKE_COMPILERS_PYC $CMAKE_COMPFLAGS_PYC $CMAKE_HDF5_CC4PY_DIR_ARG $CMAKE_SUPRA_SP_ARG $TXBASE_ADDL_ARGS $TXBASE_CC4PY_OTHER_ARGS"; then
