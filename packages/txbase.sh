...
# line-time errors in composertoolkit.
    # if bilderConfig -c txbase sersh "-DBUILD_SHARED_LIBS:BOOL=TRUE -DBUILD_WITH_SHARED_RUNTIME:BOOL=TRUE -DUSE_SHARED_HDF5:BOOL=TRUE $CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $BOOST_INCDIR_ARG $CMAKE_HDF5_SERSH_DIR_ARG $CMAKE_SUPRA_SP_ARG $TXBASE_SERSH_OTHER_ARGS"; then
# Not understood how to build shared libs on Windows
    if bilderConfig -c txbase sersh "-DBUILD_WITH_SHARED_RUNTIME:BOOL=TRUE -DUSE_SHARED_HDF5:BOOL=TRUE $CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $BOOST_INCDIR_ARG $CMAKE_HDF5_SERSH_DIR_ARG $CMAKE_SUPRA_SP_ARG $TXBASE_SERSH_OTHER_ARGS"; then
      bilderBuild txbase sersh "$TXBASE_MAKE_ARGS"
    fi
    # if bilderConfig -c txbase parsh "-DENABLE_PARALLEL:BOOL=TRUE -DBUILD_SHARED_LIBS:BOOL=TRUE -DBUILD_WITH_SHARED_RUNTIME:BOOL=TRUE -DUSE_SHARED_HDF5:BOOL=TRUE $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $BOOST_INCDIR_ARG $CMAKE_HDF5_PARSH_DIR_ARG $CMAKE_SUPRA_SP_ARG $TXBASE_PARSH_OTHER_ARGS"; then
# Not understood how to build shared libs on Windows
    if bilderConfig -c txbase parsh "-DENABLE_PARALLEL:BOOL=TRUE -DBUILD_WITH_SHARED_RUNTIME:BOOL=TRUE -DUSE_SHARED_HDF5:BOOL=TRUE $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $BOOST_INCDIR_ARG $CMAKE_HDF5_PARSH_DIR_ARG $CMAKE_SUPRA_SP_ARG $TXBASE_PARSH_OTHER_ARGS"; then
