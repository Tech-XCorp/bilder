...
buildQmcpack() {
  ...
  if bilderUnpack qmcpack; then
    ...
    if bilderConfig -c qmcpack ser "$TARBALL_NODEFLIB_FLAGS $CMAKE_COMPILERS_SER $CMAKE_COMPFLAGS_SER $CMAKE_SUPRA_SP_ARG $QMCPACK_SER_OTHER_ARGS $QMCPACK_OTHER_ARGS"; then
        bilderBuild qmcpack ser "$QMCPACK_MAKEJ_ARGS"
    fi

    if bilderConfig -c qmcpack par "-DENABLE_PARALLEL:BOOL=TRUE $TARBALL_NODEFLIB_FLAGS $CMAKE_COMPILERS_PAR $CMAKE_COMPFLAGS_PAR $CMAKE_SUPRA_SP_ARG $QMCPACK_PAR_OTHER_ARGS $QMCPACK_OTHER_ARGS"; then
