...
#
# $Id: corrcalc.sh 6766 2012-09-28 20:52:55Z cary $
#
######################################################################
...
# If python not present in other args, add the gen arg
  ...
  if bilderPreconfig corrcalc; then
	  ...
	  bilderBuild corrcalc par "$JMAKEARGS $CORRCALC_MAKE_ARGS CXX=$BUILD_DIR/corrcalc/par/txutils/cxx"
      fi
	  ...
	  bilderBuild corrcalc ser "$JMAKEARGS $CORRCALC_MAKE_ARGS CXX=$BUILD_DIR/corrcalc/ser/txutils/cxx"
      fi
  fi
