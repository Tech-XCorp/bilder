...
# Get library names
  ...
  #GEANT4_CONFIG_ARGS="${GEANT4_CONFIG_ARGS} -DGEANT4_INSTALL_DATA:BOOL=ON -DGEANT4_INSTALL_DATADIR:PATH='${G4DATA}' -DGEANT4_USE_GDML:BOOL=ON -DXERCESC_ROOT_DIR:PATH='$CONTRIB_DIR/xercesc-$FORPYTHON_SHARED_BUILD' -DXERCESC_INCLUDE_DIR:PATH='${xercescdir}/include' -DXERCESC_LIBRARY:FILEPATH='${xercescdir}/lib/${libpre}xerces-c.$libpost'"
  share/Geant4-${GEANT4_REGVER}/data
  case `uname` in
    Darwin)
