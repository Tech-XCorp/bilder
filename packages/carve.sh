...
# Carve compilers
  ...
  case `uname`-`uname -r` in
    Darwin-13.*)
      CARVE_COMPFLAGS="-DCMAKE_C_FLAGS:STRING=$PYC_CFLAGS -DCMAKE_CXX_FLAGS:STRING='$PYC_CXXFLAGS -DGTEST_HAS_TR1_TUPLE=0'"
      ;;
    *)
...
# Build the shared libs
  if bilderConfig carve $FORPYTHON_BUILD "-DCARVE_WITH_GUI:BOOL=FALSE -DBUILD_SHARED_LIBS:BOOL=TRUE -DBUILD_WITH_SHARED_RUNTIME:BOOL=TRUE $CMAKE_COMPILERS_PYC $CMAKE_COMPFLAGS_PYC -DCMAKE_BUILD_WITH_INSTALL_RPATH:BOOL=TRUE $CARVE_SERSH_OTHER_ARGS"; then
